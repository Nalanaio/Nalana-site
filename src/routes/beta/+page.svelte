<script>
  ...
  function buildTitle(build, runNumber) {
    return `Nalana Beta - ${platformLabel(build.platform)} - Build ${runNumber}`;
  }
</script>

<svelte:head>
  <title>Beta Builds — Nalana</title>
...
<main class="shell">
  ...
  {#if checkingAccount}
    <section class="card login-card">
      <span class="eyebrow">Beta channel</span>
  ...
  {:else if authorized}
    <section class="intro">
      <div>
        <span class="eyebrow">Beta testing channel</span>
        <h1>Download the latest Nalana Beta build.</h1>
        <p class="lede">These installers are published for the Nalana Beta testing program.</p>
      </div>
    ...
    {:else if data.manifest}
      ...
      <p class="note">These are Beta builds. Expect active development, and report the commit shown above with any feedback.</p>
    {/if}
  {:else}
    <section class="card login-card">
      <span class="eyebrow">Beta channel</span>
      <h1>Sign in to access beta builds.</h1>
      <p>Use the Nalana account you already have. No separate beta-page password is needed.</p>
      <a class="download-button" href="/login?next=%2Fbeta">Log in to Nalana</a>
    </section>
  {/if}
</main>
